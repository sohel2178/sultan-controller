...
export default function AdminCommands() {
  ...
  const handleCreateCommand = async () => {
    ...
    try {
      await RangsCommandAPI.createCommand({
        device_id: imei,
  ...
  const fetchCommands = async () => {
    ...
    try {
      const data = await RangsCommandAPI.fetchCommandsByIMEI(imei);
      setCommands(data);
    } catch (err) {
      console.error(err);
    }
  ...
  const generateCommands = async () => {
    ...
    try {
      setLoading(true);
      await RangsCommandAPI.generateBasicCommands(imei);
      fetchCommands();
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <Card>
        <CardHeader>
          <CardTitle>Rangs Platform Commands</CardTitle>
        </CardHeader>
