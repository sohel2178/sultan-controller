  ...
  async function waitForStackRemoval(
    ...
    for (let i = 0; i < maxRetries; i++) {
      ...
      const result: string = await new Promise((resolve) => {
        conn.exec("docker stack ps web-services", (err, stream) => {
    ...
    async start(controller) {
      ...
      conn.on("ready", async () => {
        try {
          ...
          conn.exec(
            `cd ${basePath} && docker stack rm web-services`,
            (err, stream) => {
              ...
              stream.on("close", async () => {
                ...
                conn.exec(
                  `cd ${basePath} && docker stack deploy -c docker-compose.yml web-services`,
                  (err2, stream2) => {
